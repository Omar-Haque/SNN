...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        if (total_points_processed == 105024):
            visualize_learned_templates(model, 4)
        if (total_points_processed == 150016):
            visualize_learned_templates(model, 5)

        
        if total_points_processed in [105024, 150016]:
