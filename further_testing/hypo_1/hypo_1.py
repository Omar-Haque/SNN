...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        if dominant_idx is None and total_points_processed > 5000:
            dominant_idx = torch.argmax(model.neurons.theta).item() 
        ...
        if dominant_idx is not None:
            ...
            track_weight.append(model.synapses.weight[dominant_idx].mean(dim=0).item()) 
...
if dominant_idx is not None:
    ...
    plt.plot(track_steps, track_weight, color='green', label='Synapses weight', linewidth=2)

    
    plt.title(f"Autopsy of Dominant Neuron #{dominant_idx}")
