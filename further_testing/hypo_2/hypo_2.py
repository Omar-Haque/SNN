...
weights_changed_points_history = []
weights_changed_per_timestep_history = []
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        for t in range(num_steps):
            ...
            weight_diff: torch.Tensor = torch.abs(model.synapses.weight - w_before_step)
            changed_this_step: float = (weight_diff > 1e-6).sum().item()
            batch_cumulative_weights_changed += changed_this_step
        
        total_points_processed += batch_size
        
        
        weights_changed_points_history.append(total_points_processed)
        weights_changed_per_timestep_history.append(batch_cumulative_weights_changed)
        ...
        if batch_idx % snapshot_interval == 0:
            ...
            print(f"Accuracy: {snapshot_acc:.2f}% | Cumulative Weights Changed per Timestep: {batch_cumulative_weights_changed}")
...
plt.title(f"Accuracy vs. Cumulative Weights Changed per Timestep ({model.num_neurons} Neurons)")
fig.tight_layout()
plt.show()
