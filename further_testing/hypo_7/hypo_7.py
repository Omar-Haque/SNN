...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        dominant_spiked_in_batch = False
...
if dominant_idx is not None:
    print(f"Plotting voltage plot for dominant neuron #{dominant_idx}")
    ...
    plt.title(f"Passive Neuron #{dominant_idx}")
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color='red', lw=2, label='Threshold ($\\theta$) - Spiking'),
        Line2D([0], [0], color='green', lw=2, label='Threshold ($\\theta$) - Not Spiking'),
