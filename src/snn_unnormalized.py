...
class excitatory_neurons(nn.Module):
    def __init__(self):
        ...
        self.num_neurons: int = 6400  # number of excitatory neurons.
        
        # For the adaptive threshold (in mV)
        self.theta_base:  float = -52.0                # The biological firing threshold
        self.theta_plus:  float = 0.1                  # prev: 0.1. a penalty added to the minimum fire threshold each time a neuron fires.
        self.theta_decay: float = math.exp(-1.0 / 1e5)
    ...
    def forward(self, x_exc: torch.Tensor, x_inh: torch.Tensor, learning: bool = True) -> torch.Tensor:
        ...
        if learning:
            ...
            # For biological accuracy. Empirically, the thresholds never seem to drop below -9 ish mV.
...
class DiehlAndCookNetwork(nn.Module):
    def __init__(self):
        ...
        self.lr_plus:  float = 0.003   # Learning rate for potentiation
        self.lr_minus: float = 0.00003 # Learning rate for depression
...
def visualize_learned_templates(model: DiehlAndCookNetwork) -> None:
    print("Extracting weights after training")
    ...
    # Set up a 10x10 grid for matplotlib
    fig, axes = plt.subplots(20, 20, figsize=(16, 16))
    fig.suptitle(f"Unsupervised STDP Templates ({model.num_neurons} Neurons)", fontsize=20, y=0.95)
    plt.subplots_adjust(wspace=0.05, hspace=0.05)
    
    # Loop through all neurons and plot their weight image
